 ...
 * a simplified outline for callout placement (src/data/hand-shape.json) and, with
 * --joints, the initial snap points (src/data/joints.json).
 *
...
/**
 * The right palmar hand as polygons (palm/forearm and each digit), in viewBox units.
 * Callout layout uses them to keep callouts off the hand. Other views mirror them.
 */
function handShapeJson(): string {
  const r = (pts: P[]) => pts.map((p) => toUnits(p, false)).map(([x, y]) => [Math.round(x), Math.round(y)]);
  const polygons = [r(palmPoints()), ...[LITTLE, RING, MIDDLE, INDEX, THUMB].map((d) => r(digitOutline(d)))];
  return JSON.stringify({ note: 'Generated by scripts/generate-artwork.ts. Right hand, palmar view.', polygons }) + '\n';
}
...
writeFileSync(new URL('src/data/hand-shape.json', root), handShapeJson());
console.log('Wrote src/assets/hand-palmar.svg, src/assets/hand-dorsal.svg and src/data/hand-shape.json');
